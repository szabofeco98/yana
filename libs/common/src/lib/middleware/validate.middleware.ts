...
import { CustomErrorValue, InvalidInputError } from '../exceptions/exceptions.js';
...
export const validate = (schema: ZodObject) => (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (err instanceof ZodError) {
      const errors: CustomErrorValue[] = err.issues.map(({ path, message }) => ({
        path: path.reduce((acc: string, next) => `${acc}${acc ? '.' : ''}${next.toString()}`, ''),
        message,
      }));
